% NameFile: Reversed_stateToRestMotion
...
function [time, position, velocity, acceleration, jerk, snap, crackle, pop] = Reversed_stateToRestMotion(totalTime, stopTime, accelerationValue, initialVelocity, initialPosition, intermediatePosition, constantVelocity, accelerationTime, coastingTime)
    
    % Calculate coast time
    coastTime = coastingTime;

    % Time vector
    t0 = linspace(0, stopTime, 100);
    t1 = linspace(stopTime, stopTime + accelerationTime, 100);
    t2 = linspace(stopTime + accelerationTime, stopTime + accelerationTime + coastTime, 100);
    t3 = linspace(stopTime + accelerationTime + coastTime, totalTime, 100);
    time = [t0, t1, t2, t3];

    % Acceleration profile
    a0 = ones(1, 100) * accelerationValue;
    a1 = ones(1, 100) * accelerationValue;
    a2 = zeros(1, 100);
    a3 = -ones(1, 100) * accelerationValue;
    acceleration = [a0, a1, a2, a3];

    v0 = linspace(initialVelocity, 0, 100);
    v1 = 0 + cumtrapz(t1, a1);    
    ...
    x0 = linspace(initialPosition, intermediatePosition, 100);
    x1 = intermediatePosition + cumtrapz(t1, v1);
    x2 = cumtrapz(t2, v2) + x1(end);
    x3 = x2(end) + cumtrapz(t3, v3);
    position = [x0, x1, x2, x3];
